...
//---------------------------------------------------------------------------------
void cardParamCommand2 (u8 command, u32 parameter, u32 flags, u32 *destination, u32 skip, u32 length) {
//---------------------------------------------------------------------------------
	u8 cmdData[8];
...
//---------------------------------------------------------------------------------
    u32 flags = CARD_DELAY1(0x1FFF) | CARD_DELAY2(0x3F) | CARD_nRESET | CARD_SEC_CMD | CARD_SEC_DAT | CARD_ACTIVATE | CARD_BLK_SIZE(1);
	if ( (len & 3) == 0 && (((u32)ptr) & 3) == 0 && (pos & 3) == 0) {
		// if everything is word-aligned, read directly
		cardParamCommand2(0xB7, (pos & ~0x1ff), flags, (u32*)ptr, (pos & 0x1ff) >> 2, len >> 2);
		// otherwise, use a temporary buffer
    } else {
		static u32 temp[128];
		cardParamCommand2(0xB7, (pos & ~0x1ff), flags, temp, 0, 0x200 >> 2);
